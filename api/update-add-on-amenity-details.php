...
$add_on_amenity_id = $_POST['add_on_amenity_id'];
...
$updateDetails = $db->prepare("UPDATE tb_add_on_amenities SET amenity_id = ?, quantity = ?, total_amenity_fee = ? WHERE add_on_amenity_id = ?");
$updateDetails->execute([$amenity_id, $quantity, $total_amenity_fee, $add_on_amenity_id]);

// Get po_id and recalculate total
$getPoId = $db->prepare("SELECT po_id FROM tb_add_on_amenities WHERE add_on_amenity_id = ?");
$getPoId->execute([$add_on_amenity_id]);
